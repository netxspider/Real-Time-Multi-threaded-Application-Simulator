...
export default function ThreadConfigModal({ onClose }) {
  const { threadConfigs, isRunning, isPaused, updateThreadConfig, userThreads } = useSimStore();

  // Local draft copy so changes only apply on Save
  const [draft, setDraft] = useState(() =>
    threadConfigs.map(c => ({ ...c, arrivalTime: String(c.arrivalTime), burstTime: String(c.burstTime) }))
  );
  ...
  function handleReset() {
    if (readOnly) return;
    setDraft(prev => prev.map(r => ({ ...r, arrivalTime: '0', burstTime: '5' })));
  }
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700/60">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Set arrival & burst times for each thread</p>
          </div>
          <button
            onClick={onClose}
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                ...
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-widest pb-2 pr-3">Arrival Time</th>
                <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-widest pb-2">Burst Time</th>
              </tr>
            </thead>
                  ...
                  <td className="py-2 pr-3">
                    <div className="flex items-center gap-2">
                      <span
                        ...
                        style={{ background: `hsl(${(idx * 47) % 360}, 70%, 25%)`, color: `hsl(${(idx * 47) % 360}, 70%, 70%)`, border: `1.5px solid hsl(${(idx * 47) % 360}, 70%, 50%)` }}
                      >
                        {idx + 1}
                      </span>
                  ...
                  <td className="py-2 pr-3">
                    <div className="relative">
                      <input
                        ...
                        readOnly={readOnly}
                        className={`w-full bg-gray-800 border text-gray-100 text-xs rounded-md px-2.5 py-1.5 font-mono focus:outline-none transition-colors
                          ${readOnly
